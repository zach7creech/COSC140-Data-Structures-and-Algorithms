 ...
 * COSC140 Fall 2019
 * Lab2: hflip
 * This program reads in a PGM file, determines if it valid, and if valid flips the image along the 
 * vertical axis. The resultant PGM file is printed to standard output.
 * 09/08/19
 */
...
int main()
{
	...
	//standard PGM header
	printf("P2\n%d %d\n255\n", column, row);
	//first for loop controls which row of the PGM file to parse
	for(int i = column - 1; i < totPix; i += column)
	{
		//starting from the end of each row, print until reaching the first column. Then move to the next row
